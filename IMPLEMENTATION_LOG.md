...
## 2026-07-10 — المرحلة 7: إصلاح أخير (اتجاه الأرقام RTL) + فحص شامل نهائي
- **مشكلة مكتشفة بصريًا**: في قسم "إدخال النتائج يدويًا"، كانت خلايا "الوحدة" و"المعدل المرجعي"
  تُعرض بترتيب معكوس (مثلًا "56 – 7" بدل "7 – 56") بسبب خوارزمية Unicode Bidi التي تُعيد ترتيب
  نص مختلط (أرقام + شرطة en dash) داخل سياق RTL عام. تم إصلاحه بإضافة `dir="ltr"` صراحة على تلك
  الخلايا فقط (`ResultsEntry.tsx`)، مع إبقاء بقية الجدول RTL. تم فحص باقي الصفحة للتأكد من عدم وجود
  نفس النمط في مكان آخر (اتجاهات الفروع، الباركود، أوقات واتساب) — لا مشكلة مماثلة وُجدت.
  تم توثيق هذا كسلوك يجب الانتباه له عند إضافة أي بيانات رقمية مركّبة مستقبلًا.
- فحص نهائي شامل عبر Playwright: لا أخطاء console، كل الأقسام الـ24 (Hero + 23) موجودة بمعرّفاتها
  الصحيحة، القوائم المنسدلة (Tabs/Accordion/Drawer) تعمل، حفظ/استرجاع القرارات عبر localStorage يعمل
  بعد إعادة تحميل الصفحة، تصدير JSON يعمل، الطباعة تعرض نصًا داكنًا على خلفية بيضاء مع تمييز واضح
  للقرار المختار.
- بحث نصي في كامل `src/` للتأكد من عدم وجود Lorem Ipsum أو TODO أو نصوص عرضية متروكة — لا شيء.
- `npm run build` و`npm run lint` نظيفان. Commit + push نهائي لهذه المرحلة.
